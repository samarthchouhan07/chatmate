...
import { useCreateMessage } from "../api/use-create-messsage";
...
export const Thread = ({ messageId, onClose }: ThreadProps) => {
  ...
  const { data: message, isLoading: loadigMessage } = UseGetMessage({
    id: messageId,
  });
  ...
  const groupedMessages = results?.reduce(
    (groups, message) => {
      const creationTime = message?._creationTime ?? 0;
  ...
  if (loadigMessage || status === "LoadingFirstPage") {
    return (
